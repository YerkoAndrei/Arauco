...
public class ThirdPersonOrbitCam : MonoBehaviour 
{
	...
	public float smooth = 20f;                                         // Speed of camera responsiveness.
    [SerializeField]
    private float vertiSp = verticalAimingSpeed;
	public static float horizontalAimingSpeed = 300f;                   // Horizontal turn speed.
    [SerializeField]
    private float horizSp= horizontalAimingSpeed;
	public static float verticalAimingSpeed = 300f;                    // Vertical turn speed.
	public static float maxVerticalAngle = 18f;							// Camera max clamp angle. 
	...
	public static Vector3 targetCamOffset;                                   // Camera offset target to interpolate.
	private float defaultFOV;                                          // Default camera Field of View.
	private float targetFOV;                                           // Target camera FIeld of View.
	public static float targetMaxVerticalAngle;                              // Custom camera max vertical clamp angle. 

	void Awake()
	{
		targetMaxVerticalAngle = 18f;	//cambio camara
	...
	void LateUpdate()
	{	
        vertiSp = verticalAimingSpeed;
        horizSp = horizontalAimingSpeed;

        // Get mouse movement to orbit the camera.
        angleH += Mathf.Clamp(Input.GetAxis("Mouse X"), -1, 1) * horizSp * Time.deltaTime;
		angleV += Mathf.Clamp(Input.GetAxis("Mouse Y"), -1, 1) * vertiSp * Time.deltaTime;
	...
	// Para cambiar camara
	public static void Cambiar_camara_offset(Vector3 newCamOffset, float minimo_inferior)
	{
		targetCamOffset = newCamOffset;
		targetMaxVerticalAngle = minimo_inferior;
	}
	...
	*/
	...
	{/*
		...
		if(Physics.Raycast(checkPos, player.position+(Vector3.up* deltaPlayerHeight) - checkPos, out hit, relCameraPosMag))
		{
			...
			if (hit.transform.GetComponent<CapsuleCollider> () != null) { 	// Excepcion a cuando caupolican o valdivia estan derrotados
				if (hit.transform != player && !hit.transform.GetComponent<Collider> ().isTrigger) {
					// This position isn't appropriate.
					if (hit.transform.tag == "Aliado lanza" || hit.transform.tag == "Aliado maza") // Evita que la camara se descontrole en las batallas, pero traspasa a las tropas aliadas
						return true;
					else
						return false;
				}
